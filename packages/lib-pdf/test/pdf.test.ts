...
describe("glyph widths", () => {
  const fontWith = (widths: string, extra = ""): Buffer => {
    ...
    const content = "BT /F1 10 Tf 1 0 0 1 72 720 Tm (AB) Tj ET";
    return Buffer.from(
      [
        "%PDF-1.4",
        "1 0 obj << /Type /Catalog /Pages 2 0 R >> endobj",
        ...
        `5 0 obj << /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding ` +
          `/FirstChar 65 /LastChar 66 /Widths ${widths}${extra} >> endobj`,
        "trailer << /Root 1 0 R >>",
        "%%EOF",
      ].join("\n"),
      "latin1",
  ...
  it("honours a declared width of zero instead of substituting half an em", () => {
    // A zero width is real — combining marks and the soft hyphen have one — and
    // treating it as "missing" gave the glyph 500/1000 em of advance, which is
    // enough to invent a word break the font says is not there.
    strictEqual(extractPdfText(fontWith("[0 500]")).text, "AB");
  });
