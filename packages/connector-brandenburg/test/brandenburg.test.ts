// brandenburg: the documents are public and the server's robots.txt disallows everyone.
...
import { BrandenburgSource, DOCUMENT_ORIGIN, DOCUMENT_PATH, POLITE_INTERVAL_MS, createSource } from "../src/index.js";
...
describe("brandenburg source", () => {
  ...
  it("fetches when the operator overrides, and records that on the result", async () => {
    ...
    const result = await new BrandenburgSource().discover({
  ...
  it("goes slowly when it does fetch", () => {
    ...
    strictEqual(new BrandenburgSource().minHostIntervalMs, POLITE_INTERVAL_MS);
